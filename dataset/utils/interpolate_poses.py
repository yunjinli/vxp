...
def interpolate_poses(pose_timestamps, abs_poses, requested_timestamps, origin_timestamp):
    ...
    first_pose_abs = poses_mat[0:4, 0:4]
    poses_mat_rel = np.linalg.solve(poses_mat[0:4, 0:4], poses_mat)
    # print(poses_mat)
    # poses_out = [0] * (len(requested_timestamps) - 1)
    # for i in range(1, len(requested_timestamps)):
    #     poses_out[i - 1] = poses_mat[0:4, i * 4:(i + 1) * 4]
    poses_out_rel = [0] * (len(requested_timestamps) - 1)
    for i in range(1, len(requested_timestamps)):
        poses_out_rel[i - 1] = poses_mat_rel[0:4, i * 4:(i + 1) * 4]
    # print(poses_out[0])
    # print(poses_out[1])
    return first_pose_abs, poses_out_rel
