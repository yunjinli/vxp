# Refer to https://github.com/KamilZywanowski/MinkLoc3D-SI/blob/master/datasets/samplers.py
...
# create formatter
formatter = logging.Formatter('[%(levelname)s] [%(name)s] [%(process)d] %(asctime)s: %(message)s')
# add formatter to ch
...
class BatchSampler(Sampler):
    '''
    Description:
    Sampler returns list of indices to form a mini-batch
    ...
    def __init__(self, dataset, batch_size: int, batch_size_limit: int = None,
        ...
        if self.batch_size < 2 * self.k:
            ...
            logger.warning('WARNING: Batch too small. Batch size increased to {}.'.format(self.batch_size))
    ...
    def expand_batch(self):
        if self.batch_expansion_rate is None:
            logger.warning('batch_expansion_rate is None')
            return
        ...
        logger.info('Batch size increased from: {} to {}'.format(old_batch_size, self.batch_size))
    ...
    def generate_batches(self):
        ...
        while True:
            ...
            if len(positives) == 0:
                # Broken dataset element without any positivesc
                continue
